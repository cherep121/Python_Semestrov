...
INTERESTING_FACTS_DB = {
    "russia": (
        "🇷🇺 Россия — самая большая страна в мире, занимающая 1/8 часть "
        "суши. Здесь находится самое глубокое озеро Байкал, содержащее "
        "20% всей пресной воды на планете!"
    ),
    "usa": (
        "🇺🇸 США — третья по численности населения страна в мире. Здесь "
        "находится самое большое количество миллиардеров."
    ),
    "germany": (
        "🇩🇪 Германия — первая страна в мире, где перешли на летнее время."
    ),
    "france": (
        "🇫🇷 Франция — самая посещаемая страна в мире "
        "(около 90 млн туристов в год)."
    ),
    "japan": "🇯🇵 Япония состоит из более чем 6800 островов.",
    "china": "🇨🇳 Китай изобрёл бумагу, компас, порох и книгопечатание.",
    "uk": (
        "🇬🇧 Великобритания — единственная страна, у которой нет "
        "письменной конституции."
    ),
    "india": "🇮🇳 Индия — самая большая демократия в мире.",
    "brazil": (
        "🇧🇷 Бразилия — единственная страна в Южной Америке, где говорят "
        "на португальском."
    ),
...
def get_interesting_fact(country_name):
    ...
    for key, fact in INTERESTING_FACTS_DB.items():
        if key in country_lower or country_lower in key:
            return fact
    return (
        f"🌟 {country_name} — удивительная страна с богатой историей "
...
def get_country_info_by_api(country_name):
    ...
    urls = [
        (
            ...
            f"{country_name_eng}?fullText=true"
        ),
    ...
    for url in urls:
        try:
            ...
            if response.status_code == 200:
                ...
                currency_name = (
                    list(currencies.values())[0].get("name", "N/A")
                    if currencies
                    else "N/A"
                )
...
def get_random_country():
    ...
    try:
        ...
        params = {
            "fields": (
                "name,capital,population,area,currencies,languages,"
                "flags,region,subregion"
            )
        }
        ...
        if response.status_code == 200:
            ...
            if not all_countries:
                print("API вернул пустой список стран")
                return None
            ...
            languages_name = (
                ", ".join(languages.values()) if languages else "N/A"
            )

            country_data = {
                "name": random_country.get("name", {}).get("common", "N/A"),
                "official_name": random_country.get("name", {}).get(
                    "official", "N/A"
                ),
                "capital": capital_name,
            ...
            print(
                f"Случайная страна: {country_data['name']} "
                f"(столица: {country_data['capital']})"
            )
            return country_data
        else:
...
def index():
    ...
    if request.method == "POST":
        ...
        if action == "search_by_name" and query:
            ...
            if country_data:
                country_data["description"] = get_country_description(
                    country_data["name"]
                )
                result = country_data
        ...
        # Поиск по столице
            ...
            if country_data:
                ...
                log_action(
                    username,
                    "РЕЗУЛЬТАТ ПО СТОЛИЦЕ",
                    country_data["name"]
                )
            else:
                result = {
                    "error": f"❌ Страна со столицей '{query}' не найдена"
                }
        ...
        # Сравнение двух стран
            ...
            if c1 and c2:
                ...
                log_action(
                    username,
                    "ОШИБКА СРАВНЕНИЯ",
                    f"{query} vs {compare_country2}"
                )

        # Случайная страна (ЧЕРЕЗ API)
            ...
            if country_data:
                ...
                result = {
                    "error": (
                        "❌ API временно недоступен. Попробуйте ещё раз "
    ...
    return render_template(
        ...
        username=username,
        query=query,
        compare_country2=compare_country2
    )
